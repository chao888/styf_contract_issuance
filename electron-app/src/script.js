    ...
    newItem.innerHTML = `
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <div class="input-description" style="font-size:12px;color:#999;">在 财务管理>购买记录 查询</div>
            </div>
            
            <div class="form-group">
                <label>产品/服务名称</label>
                <select name="productName[]" required onchange="handleServiceTypeChange(this)">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <div class="input-description" style="font-size:12px;color:#999;">购买系统年限或硬件数量</div>
            </div>
            
            <div class="form-group">
                <label>服务开始日期</label>
                <input type="date" name="serviceStartDate[]" placeholder="请选择开始日期">
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                <label>服务结束日期</label>
                <input type="date" name="serviceEndDate[]" placeholder="请选择结束日期">
                <div class="input-description" style="font-size:12px;color:#999;">订单中的"到期日期"</div>
            </div>
            
            <div class="form-group">
                <label>价格（元）</label>
                <input type="number" name="servicePrice[]" placeholder="请输入订单金额" required>
                <div class="input-description" style="font-size:12px;color:#999;">如有优惠，输入优惠后的"实付金额"</div>
            </div>
        </div>
    `;
...
function handleServiceTypeChange(selectElement) {
    ...
    const startDateInput = serviceItem.querySelector('input[name="serviceStartDate[]"]');
    const endDateInput = serviceItem.querySelector('input[name="serviceEndDate[]"]');
    
    const startDateGroup = startDateInput.closest('.form-group');
    const endDateGroup = endDateInput.closest('.form-group');

    const hideDateFields = [
        '青蛙PRO',
        '飞鹅小票机',
        '飞鹅小票机接入'
    ...
    console.log('Hide date fields:', hideDateFields);

    if (selectElement.value === '食堂有饭系统使用服务') {
        startDateInput.required = true;
        endDateInput.required = true;
        startDateGroup.style.display = ''; // Show
        endDateGroup.style.display = '';   // Show
        console.log('Displaying date fields for "食堂有饭系统使用服务"');
    } else if (hideDateFields) {
        startDateInput.required = false;
        endDateInput.required = false;
        startDateInput.value = ''; // Clear value when hidden
        endDateInput.value = '';   // Clear value when hidden
        startDateGroup.style.display = 'none'; // Hide
        endDateGroup.style.display = 'none';   // Hide
        console.log('Hiding date fields and clearing values');
    } else {
        startDateInput.required = false;
        endDateInput.required = false;
        startDateGroup.style.display = ''; // Show
        endDateGroup.style.display = '';   // Show
        console.log('Displaying date fields for other services');
    }
    console.log('startDateGroup display:', startDateGroup.style.display);
    console.log('endDateGroup display:', endDateGroup.style.display);
}
...
function removeServiceItem(button) {
    ...
    remainingItems.forEach((item, index) => {
        ...
        // 重新初始化日期字段的必填状态
        const selectElement = item.querySelector('select[name="productName[]"]');
        if (selectElement) {
            handleServiceTypeChange(selectElement);
        }
    });
}
const contractTemplate = {
    generate: function(data) {
        ...
        serviceItems.forEach((item, index) => {
            const startDateDisplay = formatDate(item.serviceStartDate);
            const endDateDisplay = formatDate(item.serviceEndDate);
            const dateRangeDisplay = (startDateDisplay === '--' && endDateDisplay === '--') ? '--' : `${startDateDisplay}至${endDateDisplay}`;
            ...
            serviceTableRows += `
                <tr>
                    ...
                    <td style="border: 1px solid #000; padding: 8px; text-align: center;">${dateRangeDisplay}</td>
...
// 初始化现有服务项目的日期必填状态
function initializeServiceItemRequirements() {
    document.querySelectorAll('.service-item').forEach(serviceItem => {
        const selectElement = serviceItem.querySelector('select[name="productName[]"]');
        if (selectElement) {
            handleServiceTypeChange(selectElement);
        }
    });
...
document.addEventListener('DOMContentLoaded', () => {
    setupDatePasteHandlers();
    initializeServiceItemRequirements(); // 调用初始化函数
});